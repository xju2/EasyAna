...
int main(int argc, char* argv[])
{
  ...
  if(argc!=3 && argc!=5)
    {
      std::cerr << "usage: " << argv[0] << " file_name out_name [start end]" << std::endl;
      return 1;
    }
  ...
  if(argc==5)
    {
      start=std::stoul(argv[3]);
      end  =std::stoul(argv[4]);
    }
  ...
  TH1F h_ClassOut_XGB_Higgs_Norm     ("ClassOut_XGB_Higgs_Norm"     ,"",200,0,1);
  TH1F h_ClassOut_XGB_VBF_Norm       ("ClassOut_XGB_VBF_Norm"       ,"",200,0,1);
  TH1F h_Event_MET                   ("Event_MET"                   ,"",200,0,200);
  TH1F h_Muons_Eta_Lead              ("Muons_Eta_Lead"              ,"",100,-3.0,3);
  TH1F h_Muons_Phi_Lead              ("Muons_Phi_Lead"              ,"",64,-3.2,3.2);
  TH1F h_Muons_PT_Lead               ("Muons_PT_Lead"               ,"",200,0,400);
  TH1F h_Muons_Eta_Sub               ("Muons_Eta_Sub"               ,"",100,-3.0,3);
  TH1F h_Muons_Phi_Sub               ("Muons_Phi_Sub"               ,"",64,-3.2,3.2);
  TH1F h_Muons_PT_Sub                ("Muons_PT_Sub"                ,"",200,0,400);
  TH1F h_Muons_Charge_Lead           ("Muons_Charge_Lead"           ,"",3,-1.5,1.5);
  TH1F h_Muons_CosThetaStar          ("Muons_CosThetaStar"          ,"",200,-1.0,1);
  TH1F h_Muons_Minv_MuMu             ("Muons_Minv_MuMu"             ,"",3000,0,300);
  TH1F h_Muons_Minv_MuMu_Fsr         ("Muons_Minv_MuMu_Fsr"         ,"",3000,0,300);
  TH1F h_Muons_Minv_MuMu_Fsr_nearOnly("Muons_Minv_MuMu_Fsr_nearOnly","",3000,0,300);
  TH1F h_Z_PT_FSR_nearOnly           ("Z_PT_FSR_nearOnly"           ,"",200,0,400);
  TH1F h_Z_PT_FSR_LOG_nearOnly       ("Z_PT_FSR_LOG_nearOnly"       ,"",400,-3.0,6);
  TH1F h_DiJet_Minv                  ("DiJet_Minv"                  ,"",1000,0,1000);
  TH1F h_DiJet_PT                    ("DiJet_PT"                    ,"",400,0,400);
  TH1F h_Jets_PT_Lead                ("Jets_PT_Lead"                ,"",400,0,400);
  TH1F h_Jets_Eta_Lead               ("Jets_Eta_Lead"               ,"",100,-5.0,5);
  TH1F h_Jets_PT_Sub                 ("Jets_PT_Sub"                 ,"",400,0,400);
  TH1F h_Jets_Eta_Sub                ("Jets_Eta_Sub"                ,"",100,-5.0,5);
  TH1F h_DiJet_Rapidity              ("DiJet_Rapidity"              ,"",100,-6,6);
  TH1F h_Jets_Phi_Lead               ("Jets_Phi_Lead"               ,"",64,-3.2,3.2);
  TH1F h_Jets_Phi_Sub                ("Jets_Phi_Sub"                ,"",64,-3.2,3.2);
  TH1F h_Dphi_JZ_Lead                ("Dphi_JZ_Lead"                ,"",100,-3.2,3.2);
  TH1F h_Dphi_JZ_Sub                 ("Dphi_JZ_Sub"                 ,"",100,-3.2,3.2);
  TH1F h_Z_Rapidity                  ("Z_Rapidity"                  ,"",100,-6.0,6);
  TH1F h_Jets_jetMultip              ("Jets_jetMultip"              ,"",11,-0.5,10.5);

  //
  // Prepare the reader
  TFile *fh_in=TFile::Open(file_name.c_str());
  TTree *tree=fh_in->Get<TTree>("DiMuonNtuple");
  ...
  h_ClassOut_XGB_Higgs_Norm     .Write();
  h_ClassOut_XGB_VBF_Norm       .Write();
  h_Event_MET                   .Write();
  h_Muons_Eta_Lead              .Write();
  h_Muons_Phi_Lead              .Write();
  h_Muons_PT_Lead               .Write();
  h_Muons_Eta_Sub               .Write();
  h_Muons_Phi_Sub               .Write();
  h_Muons_PT_Sub                .Write();
  h_Muons_Charge_Lead           .Write();
  h_Muons_CosThetaStar          .Write();
  h_Muons_Minv_MuMu             .Write();
  h_Muons_Minv_MuMu_Fsr         .Write();
  h_Muons_Minv_MuMu_Fsr_nearOnly.Write();
  h_Z_PT_FSR_nearOnly           .Write();
  h_Z_PT_FSR_LOG_nearOnly       .Write();
  h_DiJet_Minv                  .Write();
  h_DiJet_PT                    .Write();
  h_Jets_PT_Lead                .Write();
  h_Jets_Eta_Lead               .Write();
  h_Jets_PT_Sub                 .Write();
  h_Jets_Eta_Sub                .Write();
  h_DiJet_Rapidity              .Write();
  h_Jets_Phi_Lead               .Write();
  h_Jets_Phi_Sub                .Write();
  h_Dphi_JZ_Lead                .Write();
  h_Dphi_JZ_Sub                 .Write();
  h_Z_Rapidity                  .Write();
  h_Jets_jetMultip              .Write();
  fh_out->Close();
